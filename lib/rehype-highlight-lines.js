...
const wrapLines = function wrapLines(ast, linesToHighlight) {
  const [lastNode] = [...ast].sort((a, b) => b.lineNumber - a.lineNumber);
  const allLines = [...Array.from({ length: lastNode.lineNumber })].map((_, i) => i);
  let i = 0;
  const wrapped = allLines.reduce((nodes, marker) => {
    ...
    nodes.push({
      ...
      tagName: 'div',
      properties: {
        className: linesToHighlight.includes(line) ? 'line-highlighted' : 'line-faded',
      },
      children: children,
      lineNumber: line,
    });
...
const applyMultilineFix = function (ast) {
  ...
  // Fix JSX issue
  html = html.replace(PLAIN_TEXT_WITH_LF_TEST, (match) => {

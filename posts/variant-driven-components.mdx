...
In this article, I'll demonstrate how you can build variant-driven components using [Stitches](https://stitches.dev).
...
This is how you create and style components with Stitches. To learn more about, go have a look at the [documentation site](https://stitches.dev).
...
Currently our `Box` is 100px. But let's say we'd like to make it responsive, by increasing its size to 200px when the viewport is wider than 1000px.
...
Another approach would be to convert all the available sizes that the `Box` can exist in to variants.
...
Via props! This happens are the consumption layer, rather than the authoring layer. It looks like this:
...
In order for us to achieve this, we need to set styles based on a specific combinbation of variants. And we can do that with the compound variant API.
...
Since there are no `boxShadow` shorthands to change just its color, we're having to re-define the whole shadow config twice.
...
And here are all the possble variantions.
...
Thinking about variant-driven components can have a tremdedous positive effect on your product. Not just it enforces a more maintainable codebase, it can even improve the communication across your team.

By organising your styles in variants, you're separating its stylistic concerns. This results in less unwanted side effects.

When you combine variants together, you embrace composition. This allows for powerful UI variations.

And whenever you need a special rule for a given composition, the compound variants functionality is there to help.

As you may be able to tell, I'm very passionate about this topic. I'm glad to have spent so long thinking about variants! Some of my early exploration has contributed to the [Styled System](https://styled-system.com/)/[Theme UI](https://theme-ui.com/home) variant API. And Stitches has inspired other libraries such as [Twind](https://github.com/tw-in-js/twind) and [vanilla-extract](https://github.com/seek-oss/vanilla-extract) to follow suit.

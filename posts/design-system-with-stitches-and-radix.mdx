...
Then, import the `createCss` function and destructure and export the `style` function.
...
We created a variant called `color` and it can be either `gray` or `purple`. You can use the `defaultVariants` key to set it to `gray` by default:
...
You can use the `media` config key to define the media queries of your system. This is useful for applyin responsive styles, or for responsively applying variants.
...
You can use your predefined media queries within style object, or when applying variants. To use media queries, prefix your media keys with an `@` (at sign).
...
Alternatively, you can organise the increases as `variants`:
...
One thing I like to do is create a low-level `Box` component. It's useful because components built with Stitches support a `css` prop, meaning you have access to tokens, utils, and media queries.
...
function App() {
  return (
    <Box css={{ margin: '$2 auto' }}>
      <Box as="h1" css={{ fontSize: '24px', color: '$purple400' }}>
        Title
      </Box>
    </Box>
  );
...
I'll use [Radix]() for the more complex components, such as dialog, dropdown menu, tooltips, etc.

### Install

Radix can be installed via `yarn` or `npm`. Each primitive can be installed individually so you can adopt them incrementally.
...
For example, the focus should be automatially trapped within the dialog's content. The first interactive element should be focused by default. If there are no interactive elements, then the dialog's content should receive focus. When the dialog is closed, the focus must return to the dialog's trigger.
...
This process is similar for most Radix primitives. They're low-level enough to allow you to re-export them with your own API, yet high level enough that's an intuitive experience. It's all just components.
